...
import { directorProfileFromState, directorProfileRequiresWorkerMigration, hasLegacyDirectorTokenProfile } from './portal-runtime/access-profile.js?v=6.47.0';
import {
...
function directorProfileCard(profile, canWrite, legacyProfile = false, workerMigrationRequired = false) {
  ...
  return `<section class="card" aria-labelledby="directorProfileTitle">
    ...
    ${configured ? `<div class="admin-grid"><div class="admin-tile"><small>Método de acesso</small><strong>Senha da Diretoria</strong><small>Acesso somente leitura, sem credencial de publicação</small></div><div class="admin-tile"><small>Configurada em</small><strong>${escapeHtml(formatConfiguredAt(profile.configuredAt))}</strong><small>por ${escapeHtml(profile.configuredBy || 'Administrador')}</small></div><div class="admin-tile"><small>Segurança</small><strong>Hash derivado com PBKDF2</strong><small>A senha original nunca é armazenada ou publicada.</small></div></div>` : legacyProfile ? '<div class="notice medium is-warning"><strong>Token antigo da Diretoria detectado</strong><p>Esta versão não aceita mais token para a Diretoria. Defina uma senha abaixo e publique a alteração para liberar o novo acesso.</p></div>' : '<div class="notice medium"><strong>Nenhuma senha da Diretoria cadastrada</strong><p>Defina uma senha exclusiva para liberar o acesso completo em modo somente leitura.</p></div>'}
...
function secureStorageCard(state, canWrite) {
  ...
  return `<section class="card" aria-labelledby="secureStorageTitle">
    ...
    ${canWrite ? `<form id="secureStorageForm" class="form-grid" autocomplete="off">
      ...
      <div class="admin-security-note full-row"><span aria-hidden="true">🔐</span><div><strong>Migração segura na publicação</strong><p>Mídias públicas serão armazenadas no R2 e seus registros ficarão vinculados ao D1. Se a gravação no banco falhar, os arquivos novos são revertidos.</p></div></div>

import { cloneState, statesAreEquivalent } from '../../core/portal-state.js?v=6.49.1';
import { RESTRICTED_VIEWS } from './constants.js?v=6.49.1';
import { mergePortalStates, remotePayloadVersion } from './domain.js?v=6.49.1';
import { createAdminSessionGuard } from './session-guard.js?v=6.49.1';
import { passwordMatchesDirectorProfile } from './access-profile.js?v=6.49.1';
import {
  ACCESS_ROLES,
  accessSnapshot,
  applyAccessRole,
...
export function createAdminSessionActions(context) {
  ...
  const connectUserSession = async (username, password) => {
    ...
    const payload = await services.loadPublicGitHubPayload();
    const { authenticatePortalUser } = await import('./user-access.js?v=6.49.1');

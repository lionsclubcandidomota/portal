...
export function buildTreasuryEntryFormHtml({ item = {}, accounts = [], categories = [], isProgrammed = () => false } = {}) {
  ...
  const movementKind = normalizedItem.movementKind === 'transfer' || normalizedItem.transferGroupId
    ? 'transfer'
    : normalizedItem.movementKind === 'exit' || Number(normalizedItem.exit || 0) > 0
      ? 'exit'
      : 'entry';
  const amount = movementKind === 'entry'
  ...
  const identification = section('receipt', 'Tipo de movimentação', 'Escolha a operação primeiro. O formulário exibirá somente os campos necessários para ela.', `<div class="form-field full-row treasury-operation-picker"><div class="treasury-operation-options" role="radiogroup" aria-label="Tipo da movimentação">${operationOption('entry', 'download', 'Entrada', 'Valor que entra em uma conta do clube.')}${operationOption('exit', 'upload', 'Saída', 'Pagamento ou despesa que reduz o saldo de uma conta.')}${operationOption('transfer', 'transfer', 'Transferência', 'Move saldo de uma conta do clube para outra.')}</div><div class="treasury-operation-context" data-operation-context data-kind="${movementKind}"><span data-operation-context-icon aria-hidden="true">${uiIcon(movementKind === 'entry' ? 'download' : movementKind === 'exit' ? 'upload' : 'transfer')}</span><div><strong data-operation-context-title>${movementKind === 'entry' ? 'Registrar uma entrada' : movementKind === 'exit' ? 'Registrar uma saída' : 'Transferir entre contas'}</strong><small data-operation-context-copy>${movementKind === 'entry' ? 'O valor será somado ao saldo da conta selecionada.' : movementKind === 'exit' ? 'O valor será descontado do saldo da conta selecionada.' : 'O saldo total do clube não muda; apenas a distribuição entre contas.'}</small></div></div></div>
  ...
  const initialSubmitLabel = movementKind === 'entry' ? (normalizedItem?.id ? 'Salvar entrada' : 'Adicionar entrada') : movementKind === 'exit' ? (normalizedItem?.id ? 'Salvar saída' : 'Adicionar saída') : (normalizedItem?.id ? 'Salvar transferência' : 'Adicionar transferência');
